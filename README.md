...
4. Otsib `.bash_history` failist read, kus on `sudo`, ja salvestab need faili `/home/kasutajanimi/sudo_kasud.txt`.
5. Loob funktsiooni `varunda`, mis teeb etteantud failist varukoopia kausta `/home/kasutajanimi/backup/` kuupäeva ja kellaajaga failinimega. Testib funktsiooni `sudo_käsud.txt` peal.
...
2. Loob kataloogi `/home/kasutajanimi/` ja sinna faili `kasutajad.txt`, mille sisuks on kõik kasutajad süsteemist (`cut -d: -f1 /etc/passwd`).
...
6. Loob funktsiooni `varunda`, mis pakib kokku kogu `/home/kasutajanimi/` kataloogi ja salvestab selle `backup` kataloogi.
...
2. Loob kataloogi `/home/kasutajanimi/test` ja sinna 10 faili (`test1.txt … test10.txt`), mille sisuks on `date` ja `uptime` väljund.
...
2. Loob kataloogi `/home/kasutajanimi/logid` ja sinna faili `jooksvahetk.txt`, mille sisuks on `uptime` ja `who`.
...
2. Loob kataloogi `/home/kasutajanimi/projekt` ja sinna faili `süsteem.txt`, mille sisuks on `uname -a` ja `lsb_release -a`.
...
7. Lisab crontab’i kirje, mis varundab kogu `/home/kasutajanimi` kataloogi igal ööl kell 02:00.
...
2. Loob kataloogi `/home/kasutajanimi/andmed` ja sinna faili `disk.txt`, mille sisuks on käsu `df -h` väljund.
