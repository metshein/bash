...
3. Loob kataloogi `/home/sinukasutajanimi/andmed` ja 5 faili (`fail1.txt … fail5.txt`), mille sisuks on `hostname` ja `uptime` väljund.
...
2. Loob kataloogi `/home/sinukasutajanimi/projekt` ja sinna faili `projekt.txt`, mille sisuks on käsu `ls -l /etc` väljund.
...
5. Otsib `.bash_history` failist read, kus on `apt`, ja salvestab need faili `/home/kasutajanimi/projekt/apt_kasud.txt`.
...
2. Loob kataloogi `/home/sinukasutajanimi/kasutajad` ja sinna faili `kasutajad.txt`, mille sisuks on kõik kasutajad süsteemist (`cut -d: -f1 /etc/passwd`).
...
4. Loob kataloogi `/home/sinukasutajanimi/logid` ja salvestab sinna käskude `df -h` ja `free -m` väljundid eraldi failidesse.
5. Otsib `.bash_history` failist kõik read, kus on `nano`, ja salvestab need faili `nano_kasud.txt`.
6. Loob funktsiooni `varunda`, mis pakib kokku kogu `/home/sinukasutajanimi/` kataloogi ja salvestab selle `backup` kataloogi.
...
1. Küsib kasutajalt sisendina grupi nime. Loob grupi ja `eksam5` kasutaja, lisab kasutaja gruppi.
2. Loob kataloogi `/home/sinukasutajanimi/projekt` ja 5 faili kujul `fail1.txt … fail5.txt`, mille sisuks on käsu `uptime` väljund.
...
2. Loob kataloogi `/home/sinukasutajanimi/andmed` ja sinna faili `oluline.log`, mille sisuks on käsu `dmesg | tail -20` väljund.
...
2. Loob kataloogi `/home/sinukasutajanimi/test` ja sinna 10 faili (`test1.txt … test10.txt`), mille sisuks on `date` ja `uptime` väljund.
...
2. Loob kataloogi `/home/sinukasutajanimi/logid` ja sinna faili `jooksvahetk.txt`, mille sisuks on `uptime` ja `who`.
...
2. Loob kataloogi `/home/sinukasutajanimi/projekt` ja sinna faili `süsteem.txt`, mille sisuks on `uname -a` ja `lsb_release -a`.
...
7. Lisab crontab’i kirje, mis varundab kogu `/home/sinukasutajanimi` kataloogi igal ööl kell 02:00.
...
2. Loob kataloogi `/home/sinukasutajanimi/andmed` ja sinna faili `disk.txt`, mille sisuks on käsu `df -h` väljund.
